...
class ExecutionSummary(ExecutionSummaryBase):
    ...
    def get_http_status(self) -> int:
        ...
        Returns:
            500 if all endpoints failed, 207 if warnings, 200 if success
        """
        if self.failed == self.total_endpoints and self.total_endpoints > 0:
            return 500
        if self.has_errors:
            return 207 if self.has_warnings else 500
