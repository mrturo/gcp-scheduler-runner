...
function start_project() {
    ...
    PYTHONPATH="$(pwd)/src:$(pwd)" python src/app.py
    deactivate
}
...
function clean_cache() {
    ...
    rm -rf .pytest_cache .mypy_cache .cache dist build *.egg-info htmlcov .coverage .mutmut_results.txt .mutmut-cache
...
function code_check() {
    ...
    if command -v black >/dev/null 2>&1; then
        ...
        if [ "$ci_mode" = true ]; then
            black --check src/*.py test/*.py integration/*.py
        else
            black src/*.py test/*.py integration/*.py
        fi
    else
    ...
    if command -v isort >/dev/null 2>&1; then
        ...
        if [ "$ci_mode" = true ]; then
            isort --check-only src/*.py test/*.py integration/*.py
        else
            isort src/*.py test/*.py integration/*.py
        fi
    else
    ...
    if command -v autoflake >/dev/null 2>&1; then
        ...
        if [ "$ci_mode" = true ]; then
            autoflake --remove-all-unused-imports --remove-unused-variables --check --recursive src/*.py test/*.py integration/*.py
        else
            autoflake --remove-all-unused-imports --remove-unused-variables --in-place --recursive src/*.py test/*.py integration/*.py
        fi
    else
        echo -e "${YELLOW}⚠️  autoflake not found, skipping...${NC}"
    fi
    if [ -f ".venv/bin/pylint" ]; then
        echo -e "${GREEN}🔍 Running pylint on all Python files in src/, test/ and integration/...${NC}"
        PY_FILES=$(find src test integration -type f -name "*.py")
        if [ -n "$PY_FILES" ]; then
            ...
            echo -e "${YELLOW}⚠️  No Python files found in src/, test/ or integration/, skipping pylint...${NC}"
        fi
    else
    ...
    if command -v mypy >/dev/null 2>&1; then
        ...
        PYTHONPATH=src mypy src test integration --ignore-missing-imports
    else
...
function quality_gate() {
    echo -e "${GREEN}🚪 Running quality gate...${NC}"
    echo ""
    local code_check_passed=false
    local test_passed=false
    # Run code-check
    echo -e "${GREEN}📋 Step 1: Running code-check...${NC}"
    if bash envtool.sh code-check "$@"; then
        ...
        echo ""
        echo -e "${RED}Fix the code-check issues and try again.${NC}"
        exit 1
    fi
    echo ""
    # Run tests
    echo -e "${GREEN}🧪 Step 2: Running tests...${NC}"
    if bash envtool.sh test; then
        ...
        test_passed=false
        echo ""
        echo -e "${RED}Fix the test issues and try again.${NC}"
        exit 1
    fi
    # Check if both passed
    if [ "$code_check_passed" = true ] && [ "$test_passed" = true ]; then
        echo ""
        echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "${GREEN}🎉 Quality Gate PASSED${NC}"
        echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "${GREEN}✅ code-check: PASSED${NC}"
        echo -e "${GREEN}✅ test: PASSED (100% coverage)${NC}"
...
function mutation_check() {
    rm -rf .mutmut_results.txt .mutmut-cache
    if [ ! -d ".venv" ]; then
        echo -e "${RED}❌ The virtual environment (.venv) does not exist. Run first: bash envtool.sh install dev${NC}"
        exit 1
    fi
    echo -e "${GREEN}🧬 Running mutation testing with mutmut...${NC}"
    source .venv/bin/activate
    trap deactivate EXIT
    if ! command -v mutmut >/dev/null 2>&1; then
        echo -e "${RED}❌ mutmut is not installed in the virtual environment.${NC}"
        echo -e "${YELLOW}➡️  Install dev dependencies: pip install -r requirements-dev.txt${NC}"
        deactivate
        exit 1
    fi
    # Detect config in pyproject.toml or setup.cfg
    if grep -q '\[tool.mutmut\]' pyproject.toml 2>/dev/null || grep -q '\[mutmut\]' setup.cfg 2>/dev/null; then
        echo -e "${GREEN}⚙️  Using mutmut configuration from pyproject.toml or setup.cfg...${NC}"
        mutmut run
    else
        echo -e "${YELLOW}⚠️  No mutmut config found, using default paths: src/ (code), test/ (tests)${NC}"
        PYTHONPATH=src mutmut run --paths-to-mutate "src/" --tests-dir "test/"
    fi
    echo -e "${GREEN}📊 Mutation results:${NC}"
    mutmut results > .mutmut_results.txt
    cat .mutmut_results.txt
    # Parse Survived (N) line
    local survived_line
    survived_line=$(grep -E '^Survived[[:space:]]+\([0-9]+\)' .mutmut_results.txt | head -n1)
    if [[ $survived_line =~ \(([0-9]+)\) ]]; then
        local survivors=${BASH_REMATCH[1]}
        if [ "$survivors" -gt 0 ]; then
            echo -e "${RED}❌ Mutation testing failed: $survivors surviving mutants detected.${NC}"
            exit 2
        else
            echo -e "${GREEN}✅ Mutation testing passed: No surviving mutants!${NC}"
            exit 0
        fi
    else
        echo -e "${YELLOW}⚠️  Could not parse mutation results. Please check .mutmut_results.txt manually.${NC}"
        exit 1
    fi
}
...
function update_deps() {
    ...
    echo -e "${GREEN}⬆️  Checking for outdated dependencies...${NC}"
    source .venv/bin/activate
    pkgs=$(grep -vE '^#|^$' requirements.txt 2>/dev/null | sed 's/[[:space:]]//g' | cut -d= -f1)
    devpkgs=""
    if [ -f "requirements-dev.txt" ]; then
        devpkgs=$(grep -vE '^#|^$' requirements-dev.txt | sed 's/[[:space:]]//g' | cut -d= -f1)
    fi
    if [ -z "$pkgs" ] && [ -z "$devpkgs" ]; then
        echo -e "${YELLOW}⚠️  No packages found in requirements.txt or requirements-dev.txt to update.${NC}"
        deactivate
        return
    fi
    # Get outdated packages and their current versions (tabular output)
    outdated_table=$(pip list --outdated)
    if echo "$outdated_table" | grep -qE '^Package +Version +Latest +Type'; then
        # Parse the table, skip header lines
        needs_update=false
        while IFS= read -r line; do
            ...
            # Check requirements.txt (solo nombre de paquete)
            if grep -qE "^$pkgname==" requirements.txt 2>/dev/null; then
                needs_update=true
                break
            fi
            # Check requirements-dev.txt (solo nombre de paquete)
            if [ -f "requirements-dev.txt" ] && grep -qE "^$pkgname==" requirements-dev.txt; then
                needs_update=true
                break
            fi
        done <<< "$(echo "$outdated_table" | tail -n +3)"
        # Show pip list --outdated in table format for user
        echo "$outdated_table"
        if [ "$needs_update" = false ]; then
            # Agregar dependencias faltantes a requirements.txt con la última versión
            added_any=false
            while IFS= read -r line; do
                # Saltar encabezados
                if [[ "$line" =~ ^(Package|[- ]+)$ ]]; then
                ...
                latestver=$(echo "$line" | awk '{print $3}')
                # Si no está en requirements.txt ni requirements-dev.txt, agregarlo
                if ! grep -qE "^$pkgname==?" requirements.txt 2>/dev/null && { [ ! -f "requirements-dev.txt" ] || ! grep -qE "^$pkgname==?" requirements-dev.txt; }; then
                    echo "$pkgname==$latestver" >> requirements.txt
                    echo -e "${YELLOW}➕ Added $pkgname==$latestver to requirements.txt${NC}"
                    added_any=true
                fi
            done <<< "$(echo "$outdated_table" | tail -n +3)"
            if [ "$added_any" = true ]; then
                echo -e "${GREEN}✅ Missing outdated packages were added to requirements.txt.${NC}"
            else
                echo -e "${GREEN}✅ No listed outdated packages are present in requirements.txt or requirements-dev.txt. Nothing to update.${NC}"
            fi
            deactivate
            return
        fi
    ...
    echo -e "${YELLOW}Do you want to update all packages listed in requirements.txt? (y/n)${NC}"
    read -r confirm
    if [[ "$confirm" =~ ^([yY]|[yY][eE][sS])$ ]]; then
        echo -e "${GREEN}🔄 Updating only the packages from requirements.txt...${NC}"
        if [ -n "$pkgs" ]; then
            for pkg in $pkgs; do
                pip install --upgrade "$pkg"
            done
            echo -e "${GREEN}💾 Updating versions in requirements.txt...${NC}"
            tmpfile=$(mktemp)
            while IFS= read -r line; do
                if [[ "$line" =~ ^# || -z "$line" ]]; then
                    echo "$line" >> "$tmpfile"
                else
                    pkgname=$(echo "$line" | sed 's/[[:space:]]//g' | cut -d= -f1)
                    version=$(pip show "$pkgname" 2>/dev/null | awk '/^Version: /{print $2}')
                    if [ -n "$version" ]; then
                        echo "$pkgname==$version" >> "$tmpfile"
                    else
                        echo "$line" >> "$tmpfile"
                    fi
                fi
        ...
        if [ -f "requirements-dev.txt" ] && [ -n "$devpkgs" ]; then
            echo -e "${GREEN}🔄 Updating only the packages from requirements-dev.txt...${NC}"
            for pkg in $devpkgs; do
                pip install --upgrade "$pkg"
            done
            echo -e "${GREEN}💾 Updating versions in requirements-dev.txt...${NC}"
            tmpfile2=$(mktemp)
            while IFS= read -r line; do
                if [[ "$line" =~ ^# || -z "$line" ]]; then
                    echo "$line" >> "$tmpfile2"
                    ...
                    version=$(pip show "$pkgname" 2>/dev/null | awk '/^Version: /{print $2}')
                    if [ -n "$version" ]; then
                        echo "$pkgname==$version" >> "$tmpfile2"
                    else
                        echo "$line" >> "$tmpfile2"
                    fi
                fi
            done < requirements-dev.txt
            mv "$tmpfile2" requirements-dev.txt
        fi
        echo -e "${GREEN}✅ Dependencies updated only for the original packages.${NC}"
    else
        echo -e "${YELLOW}⏩ No updates were performed.${NC}"
    fi
    deactivate
}
...
case "${1:-}" in
    ...
    *)
        ...
        echo -e "${GREEN}  update-deps              ${NC}- Update only listed dependencies in requirements*.txt"
        echo -e "${GREEN}  mutation-check           ${NC}- Run mutation testing with mutmut"
        echo -e "${GREEN}  quality-gate [args]      ${NC}- Run quality gate"
